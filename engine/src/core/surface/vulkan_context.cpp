...
VulkanContext::VulkanContext(const VulkanSpecification& specs, std::shared_ptr<GameWindow> gameWindow)
    : m_specs(specs)
...
void VulkanContext::requestResize()
{
...
void VulkanContext::recreateSwapChain()
{
    ...
    glfwGetFramebufferSize((GLFWwindow*)m_gameWindow->getNativeHandle(), &width, &height);
...
{
    VkResult result = glfwCreateWindowSurface(m_instance, (GLFWwindow*)m_gameWindow->getNativeHandle(), nullptr, &m_surface);
...
bool VulkanContext::checkDeviceExtensionSupport(VkPhysicalDevice device, const std::vector<std::string>& deviceExtensions)
{
...
int VulkanContext::rateDeviceSuitability(VkPhysicalDevice device)
{
    ...
    if (!indices.isComplete() || !checkDeviceExtensionSupport(device, { VK_KHR_SWAPCHAIN_EXTENSION_NAME })) {
        score = 0;
    } else {
...
{
    std::vector<char> vertShaderCode = readFile("assets/shaders/vertex.spv");
    std::vector<char> fragShaderCode = readFile("assets/shaders/fragment.spv");
    ...
    VkResult graphicsPipelineResult = vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_graphicsPipeline);
